...
- News discovery (opcional, desligado por padrão): `GNEWS_KEY`, `DISCOVERY_ENABLED=false`, `DISCOVERY_DAILY_CAP=20`, `DISCOVERY_QUIET_START=1`, `DISCOVERY_QUIET_END=5`, `DISCOVERY_TOPICS=` (csv vazio = todas as 7 trilhas: tech,cinema,series,pop,atualidades,ciencia,geek). Loop roda **a cada 1h** em janela 06h–00h local (cobertura completa de 19h ativas); trilha padrão é C1 com botão "🎭 Trocar tom" no review. Cada busca combina 2 endpoints do GNews (top-headlines + search) serialmente com pausa de 1.2s entre eles (respeita o rate limit ~1 req/seg do free tier).
